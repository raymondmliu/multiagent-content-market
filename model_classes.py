...
class MemberAgent(Agent):
  ...
  def optimize_cons_util(self):
    ...
    def calc_util(x):
      ...
      def calc_direct_util(prod_id):
          return self.model.B[prod_id, self.unique_id] * np.exp(-self.model.ALPHA / x[prod_id]) \
            if x[prod_id] else 0

      # Utility obtained from other agents through influencer
      def calc_inf_util(prod_id):
        return self.model.B[prod_id, self.unique_id] * np.exp(-self.model.ALPHA *  (1/x[-2] + 1/self.model.infl_alloc[prod_id])) \
          if (x[-2] and self.model.infl_alloc[prod_id]) else 0

      direct_util = self.model.R_P * sum(calc_direct_util(i) for i in range(self.model.num_members) if i != self.unique_id)
      inf_util = self.model.R_P * sum(calc_inf_util(i) for i in range(self.model.num_members) if i != self.unique_id)
  ...
  def optimize_prod_util_limited(self):
    ...
    def calc_value(x):
      ...
      cur_prod_topics[self.unique_id] = x[0]
      ...
      value_maximized = np.exp(-self.model.ALPHA / infl_alloc[self.unique_id])  if infl_alloc[self.unique_id] else 0
    ...
    # results = [calc_value([x*0.01]) for x in range(1000)]
    # self.model.prod_topics[self.unique_id] = np.argmax(results) * 0.001
  ...
  def calc_prod_util(self):
    ...
    def B(cons_id):
      return self.model.q(self.model.prod_topics[self.unique_id], self.unique_id) * self.model.p(self.model.prod_topics[self.unique_id], cons_id)
    
    # Social support received directly from consumers
    def calc_direct_util(cons_id):
      return B(cons_id) * np.exp(-self.model.ALPHA / self.model.mems_alloc[cons_id, self.unique_id]) \
  ...
  def calc_cons_util(self):
    # Utility obtained directly from other agents
    def calc_direct_util(prod_id):
        return self.model.B[prod_id, self.unique_id] * np.exp(-self.model.ALPHA / self.model.mems_alloc[self.unique_id, prod_id]) \
          if self.model.mems_alloc[self.unique_id, prod_id] else 0

    # Utility obtained from other agents through influencer
    def calc_inf_util(prod_id):
      return self.model.B[prod_id, self.unique_id] * np.exp(-self.model.ALPHA *  (1/self.model.mems_alloc[self.unique_id, -2] + 1/self.model.infl_alloc[prod_id])) \
        if (self.model.mems_alloc[self.unique_id, -2] and self.model.infl_alloc[prod_id]) else 0

    direct_util = self.model.R_P * sum(calc_direct_util(i) for i in range(self.model.num_members) if i != self.unique_id)
    inf_util = self.model.R_P * sum(calc_inf_util(i) for i in range(self.model.num_members) if i != self.unique_id)

    # Utility from outside sources
    out_util = self.model.R_0 * self.model.B_0 * np.exp(- self.model.ALPHA / self.model.mems_alloc[self.unique_id, -1]) \
                  if (self.model.mems_alloc[self.unique_id, -1]) else 0
  ...
  def optimize_prod_util(self):
    ...
    def calc_util(x):
      ...
      if self.model.info_access == InfoAccessEnum.PERFECT: # Perfect information
        return (- direct_util - inf_util)
      elif self.model.info_access == InfoAccessEnum.LIMITED_AUTO: # Limited information: only take into account social support received from influencer
        return (- inf_util)
      else:
  ...
  def step(self):
    ...
    if self.model.info_access == InfoAccessEnum.LIMITED_MANUAL:
...
class InfluencerAgent(Agent):
  ...
  def step(self):
    ...
    for m in self.model.members:
      m.consumer_utility = m.calc_cons_util()
    if self.model.v:
...
class ContentMarketModel(Model):
  def __init__(self, params, main_topics, prod_topics, mems_alloc, infl_alloc, min_params, f, g):
    ...
    self.schedule = AlternatingScheduler(self, params['infl_update_frequency'])
